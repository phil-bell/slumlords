...
class UpdatingElement$1 extends HTMLElement {
  ...
  static getPropertyOptions(name) {
    return this._classProperties && this._classProperties.get(name) || defaultPropertyDeclaration$1;
  }
  /**
  ...
  _enqueueUpdate() {
    ...
    return _asyncToGenerator(function* () {
      _this._updateState = _this._updateState | STATE_UPDATE_REQUESTED$1;
...
LitElement$1.shadowRootOptions = {
  mode: 'open'
};
...
var toBooleanAttribute = value => value ? '' : null;
/**
 * Change function that returns true if `value` is different from `oldValue`.
 * This method is used as the default for a property's `hasChanged` function.
 */
...
var microtaskPromise = new Promise(resolve => resolve(true));
...
class UpdatingElement extends HTMLElement {
  ...
  static get observedAttributes() {
    ...
    for (var [p, v] of this._classProperties) {
      var attr = this._attributeNameForProperty(p, v);
  ...
  _saveInstanceProperties() {
    for (var [p] of this.constructor._classProperties) {
      if (this.hasOwnProperty(p)) {
        var value = this[p];
        delete this[p];
  ...
  _applyInstanceProperties() {
    for (var [p, v] of this._instanceProperties) {
      this[p] = v;
    }
  ...
  _invalidate() {
    ...
    return _asyncToGenerator(function* () {
      if (!_this._hasRequestedUpdate) {
        // mark state updating...
        _this._updateState = _this._updateState | STATE_UPDATE_REQUESTED;
        var resolver;
        var previousValidatePromise = _this._updatePromise;
        _this._updatePromise = new Promise(r => resolver = r);
        yield previousValidatePromise;

        _this._validate();

        resolver(!_this._hasRequestedUpdate);
      }
  ...
  update(_changedProperties) {
    if (this._reflectingProperties !== undefined && this._reflectingProperties.size > 0) {
      for (var [k, v] of this._reflectingProperties) {
...
class GoogleMapsLimited extends LitElement {
  ...
  constructor() {
    super(); // _mapScriptTag sets up and the google maps loader script tag - we inject it here
